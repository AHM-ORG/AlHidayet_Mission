    ...
    with open(transcript_path, 'r', encoding='utf-8', errors='ignore') as f:
        for idx, line in enumerate(f):
            if idx == 642:
                ...
                target_decoded = decode_safely(target)
                rc_decoded = decode_safely(rc)
                
                print("TargetContent (length %d):" % len(target_decoded))
                print(repr(target_decoded))
                print("\nReplacementContent (length %d):" % len(rc_decoded))
                print(rc_decoded[:300])
                print("...")
                print(rc_decoded[-300:])
                break
