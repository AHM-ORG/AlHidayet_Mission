...
def decode_val(val):
    ...
    if isinstance(val, str):
        # If it's a JSON string representation, load it
        ...
        # Now unescape the escapes using unicode_escape
        try:
            # We encode to ascii/utf-8 first, then decode using unicode_escape
            val = val.encode('utf-8').decode('unicode_escape')
        except Exception as e:
            print(f"    Failed unescaping: {e}")
            pass
    return val
...
for edit in edits:
    ...
    if name == 'replace_file_content':
        ...
        if target_norm in content_norm:
            ...
            print(f"  Target sample: {repr(target_norm[:120])}")
