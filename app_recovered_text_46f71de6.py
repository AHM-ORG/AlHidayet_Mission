    ...
    if 'user' in session and session['role'] == 'admin': # Only Admin sets subjects
        ...
        if request.method == 'POST' and 'create_subject' in request.form:
            ...
            if not classes:
                ...
                for class_name in classes:
                    # Optional: check if exists before inserting to avoid duplicates
                    existing = conn.execute("SELECT id FROM subjects WHERE name = ? AND class = ?", (name, class_name)).fetchone()
                    if not existing:
                        conn.execute("INSERT INTO subjects (name, class) VALUES (?, ?)", (name, class_name))
                conn.commit()
                flash('Subject added for selected classes!')
            conn.close()
...
def student_promotion():
    if 'user' in session and session['role'] in ['admin', 'teacher']:
        ...
        if request.method == 'POST':
            ...
            # Branch check for Branch Admins
            if session.get('branch'):
                student = conn.execute("SELECT branch FROM student_info WHERE user_id = ?", (student_id,)).fetchone()
                if not student or student['branch'] != session['branch']:
                    conn.close()
